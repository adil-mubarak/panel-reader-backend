...
func TestExternalDetectorValidResponseAndOrdering(t *testing.T) {
	...
	frames, err := a.detectPanelsFile(t.Context(), imagePath, "comic-1", 7, "ltr")
	if err != nil {
		t.Fatal(err)
	}
	if request.ComicID != "comic-1" || request.Page != 7 || request.ImagePath != imagePath || request.ReadingDirection != "ltr" {
...
func TestExternalDetectorInvalidCoordinateFallsBack(t *testing.T) {
	...
	defer server.Close()
	a, imagePath := detectorTestApp(t, server.URL)
	frames, err := a.detectPanelsFile(t.Context(), imagePath, "comic", 1, "ltr")
	if err != nil {
		t.Fatal(err)
	}
